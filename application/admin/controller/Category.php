...
class Category extends Adminbase
{
	...
	public function index(){
		...
		if (!empty($result)) {
			...
			$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ ');
			$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
			//<a href="' . url("category/add", array("parent_id" => $r['category_id'])) . '">添加子类</a> | 
			foreach ($result as $r) {
				$r['str_manage'] = '<a href="' . url("category/edit", array("id" => $r['category_id'])) . '">编辑</a> | <a class="js-ajax-delete" href="' . url("category/delete", array("id" => $r['category_id'])) . '">删除</a> ';
				$r['id']=$r['category_id'];
	...
	public function add(){
        ...
        $plat = $this->nav_model->order('nav_sort asc')->select()->toArray();
	...
	public function edit(){
		...
		if(!empty($category_id)){
			$cate_data = $this->cate_model->where(["category_id"=>$category_id])
                ->field("category_id,parent_id,catname,description,type,nav_id")
                ->find()->toArray();
		}
